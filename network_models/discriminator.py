...
class ModelDiscriminator:
    def __init__(self, env, obs_dim, act_dim):
        ...
        with tf.variable_scope('discriminator'):
            ...
            # Expert placeholders
            self.expert_s = tf.placeholder(dtype=tf.float32, shape=[None] + [obs_dim])
            self.expert_a = tf.placeholder(dtype=tf.float32, shape=[None] + [act_dim])
            expert_s_a = tf.concat([self.expert_s, self.expert_a], axis=1)

            # Generator placeholders
            self.agent_s = tf.placeholder(dtype=tf.float32, shape=[None] + [obs_dim])
            self.agent_a = tf.placeholder(dtype=tf.float32, shape=[None] + [act_dim])
            ...
            with tf.variable_scope('loss'):
                loss_expert = tf.reduce_mean(tf.log(tf.clip_by_value(prob_1, 0.1, 1)))
                #loss_agent = tf.reduce_mean(tf.log(tf.clip_by_value(1 - prob_2, 0.01, 1)))
                loss_agent = tf.reduce_mean(tf.log(tf.clip_by_value(1 - prob_2, 0.1, 1)))
                loss = loss_expert + loss_agent
                loss = -loss
                tf.summary.scalar('discriminator', loss)
            ...
            #self.rewards = tf.log(tf.clip_by_value(prob_2, 1e-10, 1))  # log(P(expert|s,a)) larger is better for agent
            self.rewards = tf.clip_by_value(prob_2, 1e-10, 1)

    def construct_network(self, input):
        activation = tf.nn.relu
        layer_1 = tf.layers.dense(inputs=input, units=64, activation=activation, name='layer1')
        layer_2 = tf.layers.dense(inputs=layer_1, units=64, activation=activation, name='layer2')
        layer_3 = tf.layers.dense(inputs=layer_2, units=64, activation=activation, name='layer3')
        layer_4 = tf.layers.dense(inputs=layer_3, units=64, activation=activation, name='layer4')
        prob = tf.layers.dense(inputs=layer_4, units=1, activation=tf.sigmoid, name='prob')
        return prob
